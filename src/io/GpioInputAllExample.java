...
public class GpioInputAllExample {
     ...
     */
    public static void ioGpio() throws PlatformAlreadyAssignedException
	{
		...
		PinPullResistance pull = CommandArgumentParser.getPinPullResistance(PinPullResistance.PULL_DOWN // default
																										// pin
																										// pull
																										// resistance
																										// if
...
//		for (Pin pin : BananaPiPin.allPins()) {
			...
			try {
				...
				provisionedPin.setShutdownOptions(true); // unexport pin on
															// program shutdown
		...
		for (GpioPinDigitalInput input : provisionedPins) {
			console.println(" [" + input.toString() + "] digital state is: "
					+ ConsoleColor.conditional(input.getState().isHigh(), // conditional
																			// expression
